...
function createStyleElement({style, name}) {
  ...
  if (!targetsList.length) {
    appliesTo.appendChild(template.appliesToEverything);
...
function findNextElement(style) {
  ...
  let a = 0;
  let b = installed.children.length - 1;
  if (b < 0) {
    return undefined;
  }
  if (b >= 0) {
    if (installed.firstElementChild.styleNameLowerCase > nameLLC) {
      return installed.firstElementChild;
    }
    if (installed.lastElementChild.styleNameLowerCase <= nameLLC) {
      return undefined;
    }
  }
